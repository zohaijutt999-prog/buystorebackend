...
async function seed() {
  ...
  try {
    ...
    // Ensure sellers table has at least one seller
    const [sellers] = await connection.query('SELECT id, shopName FROM sellers');
    let sellerIds = sellers.map(s => s.id);

    if (sellerIds.length === 0) {
      console.log('⚠️ No seller found. Creating official seller account...');
      const [res] = await connection.query(
        'INSERT INTO sellers (fullName, email, phoneNumber, shopName, password, idProofType, isApproved) VALUES (?, ?, ?, ?, ?, ?, ?)',
        ['Official Store Owner', 'official@buystore.io', '1234567890', 'Buystore Official', 'seller123', 'National ID', 1]
      );
      sellerIds = [res.insertId];
      console.log('✅ Created official seller with ID:', res.insertId);
    }

    console.log('🗑️ Deleting all existing products...');
    await connection.query('DELETE FROM products');

    console.log(`🚀 Inserting exactly 30 products for each seller (${sellerIds.length} sellers)...`);
    for (const sellerId of sellerIds) {
      for (const item of productsList) {
        const profit = Number((item.price * 0.20).toFixed(2));
        await connection.query(
          'INSERT INTO products (seller_id, title, category, price, stock_qty, description, profit, image_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
          [
            sellerId,
            item.title,
            item.category,
            item.price,
            50, // default stock
            `High quality ${item.title} with premium warranty and fast shipping.`,
            profit,
            item.img
          ]
        );
    ...
    console.log('✅ Exactly 30 products successfully added!');
    connection.release();
    process.exit(0);
  } catch (error) {
